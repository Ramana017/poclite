...
export class PayorPlanDetailsComponent implements OnInit {
    ...
    selectChange(event, field) {
        ...
        if (field === 'relation') {
            this.payorPlanForm.get('relationshipListID').setValue(event.id)
    ...
    get1(event) {
        if (event.target.checked) {
            ...
            try {
                this.payorPlanForm.get('relationshipListID').setValue(100);
                this.payorPlanForm.get('relationshipList').setValue(this.guardata.relationship);
              //  this.payorPlanForm.get('relationshipList').setValue('SELF');
               // this.payorPlanForm.get('relationshipListID').setValue(100);
